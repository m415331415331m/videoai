...
app.post("/analyze", async (req, res) => {
  ...
  try {
    ...
    const videoStats = fs.statSync(rawVideoPath);
    ...
    console.log(`[${id}] Sending video to Gemini...`);
    ...
    const videoBase64 =
      fs.readFileSync(rawVideoPath).toString("base64");
...
`;
    ...
    console.log(`[${id}] Gemini response received.`);

    const analysis = cleanJson(responseText);
    ...
    for (let i = 0; i < analysis.clips.length; i++) {
      const item = analysis.clips[i];

      let start = number(item.start, 0);
      let end = number(item.end, start + 15);
